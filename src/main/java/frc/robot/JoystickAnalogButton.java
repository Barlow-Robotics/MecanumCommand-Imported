...
public class JoystickAnalogButton extends JoystickButton {
  ...
  int m_axisNumber;
  private double minPressedValue = 0.0 ;
  private double maxPressedValue = 0.0 ;
   ...
   */
  public JoystickAnalogButton(GenericHID joystick, int axisNumber, double min, double max) {
    super( joystick, axisNumber) ;
      m_joystick = joystick;
      m_axisNumber = axisNumber;
      minPressedValue = min ;
  ...
  public boolean get() {
    ...
    return ( value >= minPressedValue && value <= maxPressedValue) ;
    // if(THRESHOLD < 0){
